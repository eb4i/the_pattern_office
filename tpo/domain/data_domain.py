...
class DomainConfig(BaseConfig):
    ...
    def build(self) -> str:
        return f"Building domain config: {self.name} ({self.url}) with abbreviation {self.abbreviation} and zone {self.zone}"

    def __str__(self) -> str:
        return f"{super().__str__()} with url {self.url}, abbreviation {self.abbreviation}, and zone {self.zone}"
